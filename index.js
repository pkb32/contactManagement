...
app.post("/identify", async (req, res) => {
  try {
    ...
    const initialMatches = await knex("contacts").where((builder) => {
    ...
    if (initialMatches.length > 0) {
      ...
      primaryContact =
        allContacts.find((c) => c.linkPrecedence === "primary") ||
        allContacts[0];
      ...
      for (const contact of allContacts) {
        if (
          ...
          await knex("contacts").where({ id: contact.id }).update({
      ...
      if (
        ...
        const finalContacts = allContacts.sort(
...
async function getAllLinkedContacts(initialMatches) {
  ...
  while (toVisit.length > 0) {
    ...
    const linked = await knex("contacts").where((builder) => {
      builder
